...
impl<'input> Routine<'input> {
    fn new(code: &str, input: &'input mut dyn Input) -> Routine<'input> {
        let buffer: Vec<char> = code
            .chars()
            .map(|x| if x.is_ascii() { x } else { '_' })
            .collect(); // filter out characters larger than 8-bit to simplify indexing
        Routine {
            code: buffer,
            position: 0,
            state: State::new(),
...
mod tests {
	...
	++++[<-------->-]<---.>++++[<----->-]<---.[-][]]",
        ...
        assert_eq!(
            char::from_u32(stream.read_value().unwrap() as u32).unwrap(),
            'H'
        );
        assert_eq!(
            char::from_u32(stream.read_value().unwrap() as u32).unwrap(),
